...
The CLI and most of its hard dependencies are included by default. However, if you want to run your application or
beautify the Typescript generated by the ``litestar schema typescript`` command, you'll need ``uvicorn`` and
``jsbeautifier`` . They can be installed independently, but we recommend installing the ``standard`` group
which conveniently bundle commonly used optional dependencies.
...
.. code-block:: shell
   ...
   poetry install litestar -E standard
